...
if (isset($_FILES['file']) && $_FILES['file']['error'] == 0)
{
	...
	// SET PREFIX NAME
	$existColumnPerma = $db->query("SHOW COLUMNS FROM ".$tables[$tabela]." LIKE 'permalink'");
	...
	if($upload_temp[0] == 'true')
	{
		...
		if($height > $width) {
			$new_width	= $TAM;
			$new_height = intval(($TAM * $height)/$width);
		} else {
			$new_height	= intval(($height / $width) * $TAM);
			$new_width	= $TAM;
		}
		...
		$image->resize($new_width, $new_height, "fit", "c", "t", $QUALITY_IMAGE, "webp");
		...
		if( !empty($wMd) && !empty($hMd) )
		{
			// MED - CREATE WEBP
			$imgMd = new Image(TEMP.$fileName);
			$imgMd->setPathToTempFiles(TEMP);
			$imgMd->resize($wMd, $hMd, "crop", "c", "c", $QUALITY_IMAGE, "webp");
		...
		$imgTb->resize($wTb, $hTb, "crop", "c", "c", $QUALITY_IMAGE, "webp");
