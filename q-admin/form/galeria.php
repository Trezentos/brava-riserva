...
<form name="form" id="form" action="" method="post" enctype="multipart/form-data" role="form">
	...
	<ul id="tab-nav" class="nav nav-tabs">
		<li <?=(isset($_POST) && $_POST['tab']==2 ? '' : 'class="active"' ) ?>><a href="#tab1" data-toggle="tab">Geral</a></li>
			...
			<fieldset>
				<div class="row">
					<div class="form-group col-md-5">
						<label>Título *</label>
						<input type="text" class="form-control input-sm" id="titulo" name="titulo" value="<?=($q ? $q->titulo : $_POST['titulo']); ?>" required/>
					</div>
					<div class="form-group col-md-5">
						<label for="permalink">Permalink *</label>
						<input type="text" class="form-control input-sm" id="permalink" name="permalink" value="<?=($q ? $q->permalink : $_POST['permalink']); ?>" required/>
					</div>
					<div class="form-group col-md-2">
						<div class="switch-wrap">
		                    <label>Status *</label>
		                    <div class="check-radio status">
		                        <input type="checkbox" name="status" value="1" <?=$q->status==1?'checked':''?> />
		                    </div>
		                </div>
					</div>
				</div>
		...
		<div id="tab2" class="tab-pane <?=($_GET["tab"]=="2"?"active":""); ?> waypoint animation_bottom animated">
			<div class="row">
				<div class="col-xs-12">
					...
					<div id="lista-imagens" class="<?=$CAT_GAL?> row"><?=get_thumbs($q->id, $__TABLE__IMG, HTTP_UPLOADS_IMG, $CAT_GAL) ?></div>
				</div>
			</div>
		</div>
	</div>
	...
	<input type="hidden" name="cat_imagens" id="cat_imagens" value="galeria" />
