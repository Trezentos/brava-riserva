...
if($_POST && isset($_POST['submit']))
{
	...
	if( $id )
	{
		...
		# INSERT IMAGE
		$upload_temp = upload('imagem',TEMP,MAX_SIZE);
		...
		if($upload_temp[0] == 'true')
		{
			...
			$image->resize(1920, 1080, "crop", "c", "c", 100, "webp");
			$image->save(ROOT_UPLOADS_IMG.$fileNewName, "", "webp");
			
			$fileNewName = $fileNewName.'.'.$extensao;
			$db->update($tables[$__TABLE__], ['imagem' => $fileNewName], ['id'=>$id]);
...
# DELETE IMAGE
if( $_GET["del_img"] == "1" && $_GET["imagem"] == "1" )
{
	deleteImg($id, $__TABLE__, "imagem", ROOT_UPLOADS_IMG);

	$db->update($tables[$__TABLE__], ['imagem' => NULL], ['id'=>$id]);
